...
# Introduction
In our previous post we explored the data structures utilized in **tm** and **tidytext**.  In this article we compare methods for cleaning and normalizing text with each package.  For this post well focus on striping numbers and puntucation, converting to lower case, tokenization and stemming or lemitization.  
...
# Cleaning With **tm**
The **tm** package provides a full set of functions to handle standard text normalization and cleaning.  The cleaning functions are invocked using the `tm_map()` function.  Custom cleaning functions can also be built using `content_transformer()`.  